...
export class SBController {
    ...
    breadCrumbUpdate(controlName: string, category: string, sampleName: string) {
        ...
        if (controlName === 'PDF Viewer') {
            ...
            const components: any = { 'spreadsheet': 'spreadsheet-editor', 'PDF Viewer': 'pdf-viewer', 'document-editor': 'docx-editor' };
            const canonicalUrl: string = `https://document.syncfusion.com/demos/${components[controlName]}/angular/#/${themeTag}/${controlTag}/${sampleTag}`;
        ...
        for (let i: number = 0, len: number = sbArray.length; i < len; i++) {
            ...
            if (sb === 'asp_core') {
                ...
                if (component === 'spreadsheet') {
                    ...
                    sampleUrl += component === 'pdfviewer' ? 'pdf-viewer' : 'document-editor';
    ...
    updateSourceCode(path: string): void {
        ...
        const isAISamples: boolean = /ai-(?!assistview\b)[a-z-]+/.test(pathArray[0]) || (pathArray[0] === 'spreadsheet' && (/^ai-.*/).test(pathArray[1]));
